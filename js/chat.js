...
function updateUserProfile() {
    ...
    // Update trusted status - 10,000 RP required
    if (userData.rp >= 10000) {
...
function sendMessage() {
    ...
    if (userData.rp >= 10000 && !userData.isTrusted) {
        showSystemMessage('🎉 Congratulations! You reached 10,000 RP and unlocked Trusted role!');
        imageBtn.disabled = false;
    }
}
...
if (imageBtn) {
    imageBtn.addEventListener('click', () => {
        if (!userData.isTrusted) {
            alert('Images require Trusted role (10,000 RP)');
            return;
        }
